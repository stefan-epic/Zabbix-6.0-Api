...
internal class Check
{
    ...
    public static void IsNotNullOrWhiteSpace(string value, string name = null)
    {
        if (string.IsNullOrWhiteSpace(value))
            throw new ArgumentNullException(string.Format("The field {0} is null or empty", name));
    }
    ...
    */
    public static void EnumerableNotNullOrEmpty<T>(IEnumerable<T> value, string name = null)
    {
        NotNull(value, name);
        if (!value.Any())
